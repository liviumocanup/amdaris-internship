...
string text = @
"
In object-oriented programming, encapsulation is a fundamental principle that involves bundling data and methods that operate on that data within a single unit or class. 
...
By encapsulating data and methods together, we promote code reusability, maintainability, and flexibility.
One of the key benefits of encapsulation is the ability to enforce access control on the members of a class. 
...
Encapsulation also allows us to enforce data validation and maintain invariants within our classes. 
By providing controlled access to data through methods, we can ensure that it is always in a valid state. 
...
int wordOccurences = words.Count(x => x.Equals("encapsulation", StringComparison.OrdinalIgnoreCase));
Console.WriteLine($"'Encapsulation' word occurences: {wordOccurences}");
...
Console.WriteLine(string.Format("String in reverse:\n```\n{0}\n```", reverseBuilder));



//- In the given string, replace all occurances of "object-oriented programming" with "OOP" and display the new string
string wordReplace = text.Replace("object-oriented programming", "OOP");
Console.WriteLine(string.Format("String with OOP replaced:\n```\n{0}\n```", wordReplace));
